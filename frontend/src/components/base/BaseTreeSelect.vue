...
const emit = defineEmits<{
  ...
  (e: 'change', item: TreeSelectItem): void
}>()
...
// 弹层 Teleport 到 body，避免被祖先 overflow 裁剪；宽度跟随触发器
const { panelStyle, isOutside } = useDropdownPosition(open, rootRef, panelRef, { matchWidth: 'exact' })

function hasChildren(item: TreeSelectItem) {
  return Array.isArray(item.children) && item.children.length > 0
}

// 递归查找选中节点，用于回显 label
function findNode(items: TreeSelectItem[], value: string | number): TreeSelectItem | null {
...
function findAncestors(items: TreeSelectItem[], value: string | number, trail: Array<string | number>): Array<string | number> | null {
  for (const item of items) {
...
function onSelect(item: TreeSelectItem) {
    ...
    <Teleport to="body">
      ...
      >
        <div
          ref="panelRef"
          :style="panelStyle"
          class="rounded-lg border border-slate-200 bg-white/95 p-2 shadow-lg shadow-slate-200/60 dark:border-white/[0.08] dark:bg-[#1f1f22] dark:shadow-black/40"
        >
