...
type QueryHookOptions<TData = unknown> = {
  isKeyword?: boolean
  afterQuery?: (data: TData) => void | Promise<void>
}
...
export function usePaginatedList(fetchFn, { pageSize: defaultPageSize = 10, filterDebounce = 300, keywordDebounce = 500 } = {}) {
   ...
   */
  const watchFilters = (source, buildParams, { isKeyword = false, afterQuery }: QueryHookOptions = {}) => {
   ...
   */
  const watchPage = (buildParams, { afterQuery }: Omit<QueryHookOptions, 'isKeyword'> = {}) => {
    watch(page, async () => {
      const data = await doQuery(buildParams())
      afterQuery?.(data)
    })
