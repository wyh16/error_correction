...
def _get_embedding_client():
    ...
    优先使用专门的 RAG Embedding 配置，若未配置则复用 OpenAI provider。
    """
    from core.config import settings

    # 1. 尝试显式配置的 Embedding 凭据
    api_key = settings.rag_embedding_api_key
    base_url = settings.rag_embedding_base_url

    # 2. 若未显式配置，则回退到 OpenAI provider
    if not api_key:
        try:
    ...
    try:
        ...
        if base_url:
            # 兼容性处理：如果 base_url 指向 DeepSeek，则强行置空（DeepSeek 无 embedding 接口）
            # 除非用户显式在 RAG_EMBEDDING_BASE_URL 中指定了 DeepSeek（这种情况不应该发生）
