...
import { ALLOWED_HTML_TAGS } from './html'

/** 在 Markdown 渲染前暂存数学公式，避免被 `marked` 改写。 */
const protectMathMarkdown = (text) => {
  const placeholders: string[] = []
  if (!text) return { text: '', restore: (html) => html }
  const codePlaceholders: string[] = []
  /** 暂存代码片段，避免代码中的 `$` 被识别成数学公式。 */
  ...
  let s = String(text)
  // 先暂存代码块和行内代码，避免其中的 `$` 或 LaTeX 被误判成公式。
  s = s
  ...
  // 部分模型会直接输出裸 LaTeX 命令，这里补成 `$...$` 以便 MathJax 识别。
...
export const renderMarkdown = (text) => {
  if (!text) return ''
  // `marked` 通过 CDN 注入到 `window`；缺失时退化为直接返回原文。
  const parse = window.marked?.parse ?? ((s: string) => s)
